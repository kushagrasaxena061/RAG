...
class RAGPipelineOrchestrator:
    ...
    def _is_conversational(self, query: str) -> bool:
        """Fast-path check to avoid running expensive RAG on greetings."""
    ...
    def process_query(self, user_query: str, mock_mode: bool = False) -> Dict[str, Any]:
        ...
        # 1. Fast Path for Chit-Chat / Simple Greetings
        if self._is_conversational(user_query):
            ...
            telemetry = QueryTelemetry(
                ...
                retrieval_rounds=0, retrieved_chunks=0, final_chunks_used=0, compression_ratio=0.0
            )
            self.logger.log(telemetry)
            return {"query_id": query_id, "answer": answer, "telemetry": telemetry.model_dump()}

        # 2. Contextual Query Formation with Bounded Short-Term Memory
        ...
        # 3. Direct Hybrid Search (Avoids Planner latency unless multi-step is needed)
        # Search directly with the user query first
        raw_chunks = self.retriever.search(user_query, top_k=6)
        
        # 4. Hardware-Accelerated Reranking & Budget Compression
        reranked = self.reranker.rerank(user_query, raw_chunks, top_k=4)
